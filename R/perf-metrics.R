...
#'
#' @param signal.bmk xts object, signal with DE rep
#' @param n int number of resampling (permutation)
...
#'   metric <- rownames(metrics.random)[4]
#'   hist(metrics.random[metric, ], breaks = 300, main = paste("Hist: ", metric, sep = "")); abline(v = metrics.random[metric, 1])
#'   stats::ecdf(metrics.random[metric, ])(metrics.random[metric, 1])
#' @export
...
GenWeights <- function(signal.pack, signal.rep = c('UD', 'DE'), inits = NULL, returns) {
  ...
  lapply(signal.pack, GenFunc, names = colnames(returns), us.init = inits[1], dm.init = inits[2])
}
...
GenTrets <- function(weights, # obtained by GenWeights
  ...
  lapply(weights, function(weight) { # just return the trets list
    ...
    # apply subperiod, a vector of strings that can used for indexing
    if (!is.null(sub.period)) {
      if (returns.period == 'Monthly') {
        tret <- tret[sub.period] # apply subsetting function
      } else {
        stop("Sub period can only be used for monthly data for now")
      }
    }
...
#' Generate complete outputs for a pack of signals (weights and total returns), assumed first bmk
#'
#' @param weights a list of xts dataframes, representing weights time series, generated
#'   from GenWeights, with names properly set as signal.names
...
#' @export
GenCoreMetrics <- function(total.ret,
                           benchmark.ret = total.ret,
  ...
  # up/down captures
  # use benchmark.ret to seperate 2 groups of months
  up.index <- benchmark.ret > 0
  up.cap <- sum(total.ret[up.index]) / sum(benchmark.ret[up.index])
  down.cap <- sum(total.ret[!up.index]) / sum(benchmark.ret[!up.index])

  metrics <- c('Mean' = m * 100,
               'Stdev' = sd * 100,
               ...
               'MaxDD' = maxDD * 100,
               'Worst' = worst.ret * 100,
               'Mean vs BMK' = active.m * 100,
               'TE vs BMK' = active.sd * 100,
               'IR' = information.ratio,
               'Up Capture' = up.cap * 100,
               'Down Capture' = down.cap * 100,
               '% >= BMK' = active.positive.pctg * 100,
               'MaxDD vs BMK' = active.maxDD * 100,
               'Worst vs BMK' = active.worst.ret * 100)
  list(metrics, pnl)
}
...
#'
#' @param bop.weight xts object of Beginning of Period Weights,
#'   each column represents one assets
#' @param returns xts object of log returns, same periodicity and size
#'   as bop.weight
#' @return average monthly two-way turnover rate
#' @export
GenTurnover <- function(bop.weight, returns) {

  # calculate End of Period weights
  eop.weight.raw <- bop.weight * exp(returns)
  eop.weight <- eop.weight.raw / rowSums(eop.weight.raw)
  stopifnot(all(abs(rowSums(eop.weight) - 1) < 0.0000001))

  # calculate chagnes in weights when rebalancing
  trading <- na.omit(bop.weight - xts::lag.xts(eop.weight))
  twoway.to <- rowSums(abs(trading))  # sum long and short
  mean(twoway.to) * 12  # default: annualized monthly
}
...
#' @param us.init numeric initial weight of us market in overall
#'   portfolio
#' @param dm.init numeric initial weight of dm in international
...
#'   monthly return
#' @export
GenWsUD <- function(UD.signal, names, us.init = 0.6, dm.init = 6/7) {
  us.signal <- UD.signal[, 1]
  dm.signal <- UD.signal[, 2]

  us.ws <- us.signal * (1 - us.init) + us.init # between init.US and 1
  dm.ws <- dm.signal * (1 - dm.init) + dm.init
  intl.ws <- cbind(dm.ws, 1 - dm.ws) # weights in intl portfolio

  global.ws <- cbind(us.ws, apply(intl.ws, 2, function(x)
    x * as.vector(1 - us.ws)))
  # bug fixed: first vectorize the xts to avoid type matching
  # us.ws will take care of the index if they have same nrow

  stopifnot(all(apply(global.ws, 1, sum) - 1 < 0.0000001))

  # the weights series has the same index as signal inputs
...
#' @param names colnames of output, should be sth like colnames(monthly.ret)
#' @param us.init,dm.init numeric initial weights of us in global, dm in intl
#' @export
GenWsDE <- function(DE.signal, names, us.init = 0.6, dm.init = 6/7) {
  dm <- DE.signal[, 1]
  em <- DE.signal[, 2]

  us.ws <- us.init + dm * (1 - us.init) * dm.init +
    em * (1 - us.init) * (1 - dm.init)
...
IfPeriodEqual <- function(A, B) {
  (xts::periodicity(A)$start
   == xts::periodicity(B)$start) &
    (xts::periodicity(A)$end
     == xts::periodicity(B)$end)
}
